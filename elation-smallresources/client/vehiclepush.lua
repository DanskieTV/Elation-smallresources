...
-- Ensure Config exists and has required values
Config = Config or {}
Config.DamageNeeded = Config.DamageNeeded or 1000.0
Config.PushDistance = Config.PushDistance or 3.0
Config.PushSpeed = Config.PushSpeed or 1.0
...
RegisterNetEvent('vehiclepush:client:push', function(veh)
    if veh and DoesEntityExist(veh) then
        ...
        if not engineHealth then return end
        
        if not IsEntityAttachedToEntity(ped, veh) and IsVehicleSeatFree(veh, -1) and engineHealth <= Config.DamageNeeded and engineHealth >= 0 then
            if vehClass ~= 13 or vehClass ~= 14 or vehClass ~= 15 or vehClass ~= 16 then
                NetworkRequestControlOfEntity(veh)
                if #(pos - vehPos) < Config.PushDistance and not IsPedInAnyVehicle(ped, false) then
                    ...
                    while true do
                        ...
                        SetVehicleForwardSpeed(veh, isInFront and -Config.PushSpeed or Config.PushSpeed)
...
-- Initialize target options for vehicle pushing using our bridge
CreateThread(function()
    Wait(1000) -- Wait for TargetBridge to be loaded
    if TargetBridge then
        TargetBridge.addGlobalVehicle({
            {
                name = 'push_vehicle',
                icon = 'fas fa-wrench',
                label = 'Push Vehicle',
                ...
                canInteract = function(entity)
                    ...
                    if not engineHealth then return false end
                    
                    return engineHealth <= Config.DamageNeeded and engineHealth >= 0
                end
            }
        })
    end
